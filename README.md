# extend-service-extension-with-task-scheduler-go
...
app written in `Go`. It includes an example of a custom task service which tracks 
task execution count and integrates with the Task Scheduler Service. Additionally, 
...
- The Task Scheduler Service sends an `ExecutionContext` with message type `TASK_START`
- Your service receives this event and can perform custom logic (e.g., incrementing 
  a task execution counter)
...
│   │   ├── taskSchedulerService.go    # Task Scheduler Service implementation
│   │   └── ...
│   └── ...
└── ...
```
...
1. Create a docker compose `.env` file in the project root directory.
...
2. Fill in the required environment variables in `.env` file as shown below.
   ...
   BASE_PATH='/task'                         # The base path used for the app
   ```
...
3. The REST API service can then be tested by opening Swagger UI at 
   `http://localhost:8000/task/apidocs/`. Use this to create an API request 
   to try the endpoints.
   
   > :info: Depending on the envar you set for `BASE_PATH`, the service will 
   have different service URL. This is how it's formatted: 
...
7. You can query the task execution count by calling the `GetTaskExecutionCount` endpoint through Swagger UI:
   ...
   - Navigate to the `POST /v1/tasks/execution/count` endpoint
   - Click "Try it out"
